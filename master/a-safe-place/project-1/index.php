<?php header('Content-type: text/html; charset=utf-8');
	 ...
	 *
	 */
	$option['gpa'] 						= false; // If true generate aliases.php file, else include it (php error + include it benchmark)
	...
	//generate a full dictionary of each page in a new bit language (more compressed of any other human languages)
	$option['gen_bit_dictionary']  		= true;
...
/////////////// Nothing to edit after ////////////////////////
	...
	if($option['gen_bit_dictionary'] === true)
	{
		$time_start = microtime(true);

		require_once(SYSPATH.'core/dicoGenerator.php');

		$time_end = microtime(true);

		$time = $time_end - $time_start;

		echo '<div style="position:fixed;top:0;right:0;left:0;background:green;color:white;padding:10px">Gen dictionary: Did it in '.$time.' seconds</div>';
	}
/*
 *
 * And away we go...
